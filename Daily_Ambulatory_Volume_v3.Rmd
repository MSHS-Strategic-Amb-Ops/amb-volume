...
# scheduling_data_raw <- readRDS("/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/historical_data_Jan21Dec22.rds") %>%
#   # filter(!is.na(Campus)) %>%
...
prev_encounters <- as.numeric((vol_trend_mshs %>% 
  filter(Appt.Year == as.numeric(last_year)) %>% 
  filter(Appt.Month == format((lubridate::floor_date(max(scheduling_data$Appt.DateYear), unit = "month")-1), "%B")))$monthly_vol)
