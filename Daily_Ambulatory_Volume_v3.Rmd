...
#   filter(Appt.DateYear <= as.Date(Sys.time(), format="%Y-%m-%d")-1 & Appt.Year >= as.numeric(format(Sys.time(), format="%Y"))-1)


# scheduling_data_raw <- readRDS("/data/Ambulatory/Data/historical_data.rds") %>%
#   filter(!is.na(Campus)) %>%
#   filter(Campus %!in% c("OTHER", "MSQ", "MS NOW", "NYEE"))
...
scheduling_data_raw <- as.data.frame(readRDS("/nfs/data/Applications/Ambulatory/Data_Updated/historical_data.rds")) %>%
  ...
  filter(Campus %!in% c("OTHER", "MSQ", "MS NOW", "NYEE")) %>%
  filter(Appt.Status == "Arrived")%>%
  filter(Appt.DateYear <= as.Date(Sys.time(), format="%Y-%m-%d")-1)

scheduling_data_raw <- scheduling_data_raw %>% distinct()
...
avg_2019_ref <- data.frame(
  Campus = c("MSHS", "MSBI", "MSDD", "MSH- AMBULATORY CARE", "MSH-MSDFP",
...
past_5_wks <- tail(weekNum_sunday, 6)
...
#   filter(Appt.DateYear <= "2022-03-29") 
...
if(format(max(scheduling_data$Appt.DateYear), format = "%A") %in% c("Sunday")){
...
monthly_vol_mshs$perc_diff <- percent((monthly_vol_mshs$`2022` - monthly_vol_mshs$`2021`)/monthly_vol_mshs$`2021`,1)
...
aggregate_row <- monthly_vol_mshs %>%
  ...
  dplyr::select(Appt.MonNum, Appt.Month, '2021_Total', '2022_Total', 'perc_diff_Total',
         '2021_IN PERSON', '2022_IN PERSON', 'perc_diff_IN PERSON', 
         '2021_TELEHEALTH', '2022_TELEHEALTH', 'perc_diff_TELEHEALTH')
...
col_names <- c("Month", rep(c("2021","2022","% Difference"),3))
...
vol_trend_mshs <- monthly_vol %>%
  ...
  #   
  # )
# vol_trend_mshs$perc_diff <- (vol_trend_mshs$`2022` - vol_trend_mshs$`2021`)/vol_trend_mshs$`2021`
...
# Reference Lines
avg_2021 <- round(mean((vol_trend_mshs %>% filter(Appt.Year == "2021"))$monthly_vol),0)
...
g1 <- ggplot(vol_trend_mshs, 
  ...
  scale_y_continuous(limits = c(0, max(max(vol_trend_mshs$monthly_vol),avg_2021)*1.2), labels = scales::number_format(accuracy = 1,
                                 decimal.mark = ','))+
  labs(title = "MSHS Ambulatory Arrived Volume by Month",
       subtitle = paste0("Based on data from 2021-01-01 to ", max(scheduling_data$Appt.DateYear),"\n"),
  ...
  geom_hline(yintercept=avg_2021, linetype = "dashed") +
   annotate(geom="text", label=paste0("2021 Monthly Average:\n", prettyNum(avg_2021, big.mark = ',')), x=11, y=avg_2021, vjust=-1, size=3.5, fontface="bold")+
...
# Volume Breakdown by Month 
    ...
    #                    Appt.Year = factor("2022",levels = c("2021","2022")), Visit.Method = "Total")
    ann_text_2021 <- data.frame(Appt.Month = "May",monthly_vol = avg_2021,lab=paste0("2021 Monthly\nAvg: ", prettyNum(avg_2021, big.mark = ',')),
                       Appt.Year = factor("2022",levels = c("2021","2022")), Visit.Method = "Total")
   ...
   ggplot(vol_breakdown_mshs, aes(x= factor(Appt.Month, levels = monthOptions), 
     ...
     # geom_text(data = ann_text_2019, aes(label = lab), vjust=-0.5, size=3.5, fontface="bold")+
          geom_hline(yintercept=avg_2021, linetype = "dashed") +
          geom_text(data = ann_text_2021, aes(label = lab), size=3, fontface="bold", vjust = -1, nudge_x = 1)+
        labs(caption = paste0("*2019 Monthly Average: ", prettyNum(avg_2019, big.mark = ','),"\n**Includes Rad Onc data up to ", rad_onc_date, " and Radiology data up to ",radiology_date))+
      theme(plot.caption = element_text(hjust = 0, vjust = 0, face="italic", size = 10),
            plot.margin = margin(10,10,10,10))
  ...
  # geom_hline(yintercept=avg_2021, linetype = "dashed") +
  #  annotate(data = (vol_breakdown_mshs %>% filter(Appt.Year == max(Appt.Year))), geom="text", label=paste0("2021 Monthly Average: ", prettyNum(avg_2021, big.mark = ',')), 
  #           x=(length(unique((vol_breakdown_mshs %>% filter(Appt.Year == max(Appt.Year)))$Appt.Month))-1), y=avg_2021, vjust=-1, size=3.5, fontface="bold")
...
prev_encounters <- as.numeric((vol_trend_mshs %>% 
  ...
  filter(Appt.Month == format((lubridate::floor_date(max(scheduling_data$Appt.DateYear), unit = "year")), "%B")))$monthly_vol)

total_encounters_comp <- percent(current_encounters/prev_encounters,0)
  
total_in_person <- percent((monthly_vol_mshs %>% filter(Visit.Method == "IN PERSON") %>% filter(Appt.Month == full_month))$perc_diff + 1,0)

total_amb_prior_year <- percent(current_encounters/avg_2021,0)
total_amb_prev_year <- percent(current_encounters/avg_2019,0)
...
<font size="4">--  Total ambulatory encounters were **`r total_encounters`**, reaching **`r total_encounters_comp`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r total_in_person`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r total_amb_prior_year`** of `r last_year` monthly average and **`r total_amb_prev_year`** of 2019 monthly average.</font><br/>
::::
 
 
### MSHS Volume by Specialty
...
monthly_vol_mshs_specialty <- merge(monthly_vol_mshs_specialty, ytd_vol_mshs_specialty, by = c("Campus.Specialty","Campus"), all = TRUE)
...
# monthly_vol_mshs_specialty_trend <- monthly_vol %>%
#   filter(Appt.Year == todays_year) %>%
#   filter(Visit.Method == "Total") %>%
#   ungroup() %>%
#   group_by(Campus.Specialty, Campus, Appt.Year, Appt.MonNum, Appt.Month, Visit.Method) %>%
#   summarise(monthly_vol = sum(monthly_vol, na.rn = TRUE)) %>%
#   ungroup() %>%
#   dplyr::select(Campus.Specialty, Campus, Appt.Month, Appt.MonNum, monthly_vol) %>%
#   group_by(Campus.Specialty, Appt.Month) %>%
#   bind_rows(summarise(.,
#                       across(where(is.numeric), sum),
#                       across(where(is.character), ~paste0(Campus.Specialty, " Total")))) 
# 
# monthly_vol_mshs_specialty_trend <- unique(monthly_vol_mshs_specialty_trend)
# monthly_vol_mshs_specialty_trend <- monthly_vol_mshs_specialty_trend %>%
#   group_by(Campus.Specialty, Campus) %>%
#   summarise(monthly_vol_trend = list(monthly_vol))
# 
# monthly_vol_mshs_specialty <- merge(monthly_vol_mshs_specialty, monthly_vol_mshs_specialty_trend)
# 
# monthly_vol_mshs_specialty[is.na(monthly_vol_mshs_specialty)] <- 0
# 
# monthly_vol_mshs_specialty <- monthly_vol_mshs_specialty %>%
#   mutate(order = str_detect(Campus, "Total")) %>%
...
monthly_vol_site$perc_diff <- percent((monthly_vol_site$`2022` - monthly_vol_site$`2021`)/monthly_vol_site$`2021`,1)
...
vol_trend_site_graph <- function(site){
  ...
  avg_2021 <- round(mean((vol_trend_site %>% filter(Appt.Year == "2021"))$monthly_vol))
...
graph <- ggplot(vol_trend_site, 
  ...
  geom_point(aes(color=Appt.Year), size=3)+
   scale_colour_manual(values = c("#212070","#d80b8c"))+
  scale_y_continuous(limits = c(0, max(max(vol_trend_site$monthly_vol),avg_2021)*1.2), labels = scales::number_format(accuracy = 1,
                                 decimal.mark = ','))+
...
vol_breakdown_site_graph <- function(site){
   ...
   #    arrange(Appt.MonNum)
    ...
    vol_breakdown_site <- monthly_vol %>%
      ...
      avg_2021 <- round(mean((vol_breakdown_site %>%
    ...
    max <- vol_breakdown_site %>% group_by(Appt.Year, Appt.Month) %>% summarise(max = sum(monthly_vol))
      ...
      ann_text_2021 <- data.frame(Appt.Month = "May",monthly_vol = avg_2021,lab=paste0("2021 Monthly Average:\n", prettyNum(avg_2021, big.mark = ',')),
                       Appt.Year = factor("2022",levels = c("2021","2022")), Visit.Method = "Total")
   ...
   ggplot(vol_breakdown_site, aes(x= factor(Appt.Month, levels = monthOptions), y=monthly_vol, group=Visit.Method, fill=Visit.Method))+
      ...
      stat_summary(fun.y = sum, vjust = -1, aes(label=ifelse(..y.. == 0,"",prettyNum(..y..,',')), group = Appt.Month), geom="text", color="black", 
                   size=3, fontface="bold.italic")+
    geom_hline(yintercept=avg_2021, linetype = "dashed") +
     geom_text(data = ann_text_2021, aes(label = lab), size=3, fontface="bold", vjust = -1, nudge_x = .5)+
             labs(caption = paste0(
               # "*2019 Monthly Average: ", prettyNum(avg_2019, big.mark = ','),
               "*Includes Rad Onc data up to 02-2022 and Radiology data up to 03-2022"))+
      labs(caption = paste0("*",site," 2019 Monthly Average: ", prettyNum((avg_2019_ref$avg_2019[which(avg_2019_ref$Campus == site)]), big.mark = ','),
                            "\n**Includes Rad Onc data up to ", rad_onc_date, " and Radiology data up to ",radiology_date))+
      theme(plot.caption = element_text(hjust = 0, vjust = 0, face="italic", size = 10),
            plot.margin = margin(10,10,10,10))
...
vol_takeaways_function <- function(site){
  ...
  # site <- "MSH-MSDFP"
    ...
    total_encounters_comp_site <- percent(current_encounters_site/prev_encounters_site,0)
    
    total_in_person_site <- percent((monthly_vol_site %>% filter(Visit.Method == "IN PERSON") %>% filter(Appt.Month == full_month) %>%
                                       mutate(perc_diff =  `2022`/`2021`))$perc_diff,0)

    avg_2021_site <- round(mean((monthly_vol_site %>% filter(Visit.Method == "Total"))[[4]]))

    total_amb_prior_year_site <- percent(current_encounters_site/avg_2021_site,0)
    total_amb_prev_year_site <- percent(current_encounters_site/(avg_2019_ref$avg_2019[which(avg_2019_ref$Campus == site)]),0)
...
```{r Monthly Volume by Department, echo = FALSE, warning = FALSE, message = FALSE}
...
monthly_vol_dept <- merge(monthly_vol_dept, ytd_vol_dept, by = c("Campus.Specialty","Department"), all = TRUE)
...
reactable(monthly_vol_dept,
          ...
          columnGroups = list(
            ...
            colGroup(name = "TELEHEALTH", columns = c("monthly_vol_TELEHEALTH", "last_year_TELEHEALTH", "perc_diff_TELEHEALTH"),
                     headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px")),
            colGroup(name =  paste0(full_month," YTD Total"), columns = c(as.character(as.numeric(todays_year)-1),todays_year,"ytd_var"),
                     headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "14px"))
            ),
          columns = list(
            Campus.Specialty = colDef(
...
<font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "MSBI")[2]`**, reaching **`r vol_takeaways_function(site = "MSBI")[3]`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "MSBI")[4]`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "MSBI")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "MSBI")[6]`** of 2019 monthly average.</font><br/>
::::
_________________________________________________________________________________________________________________________________________________________________
...
## MSDD {.tabset .tabset-fade .tabset-pills}
### MSDD Daily Volume
```{r MSDD Metrics Output1, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
print(daily_vol_site_function(site = "MSDD"))
print(daily_vol_last_year_site_function(site = "MSDD"))

```
_________________________________________________________________________________________________________________________________________________________________

### MSDD Weekly Volume
```{r MSDD Metrics Output2, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
print(weekly_vol_site_function(site = "MSDD"))
print(weekly_vol_last_year_site_function(site = "MSDD"))
```
_________________________________________________________________________________________________________________________________________________________________

### MSDD Monthly Volume
```{r MSDD Metrics Output3, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
print(monthly_vol_site_function(site = "MSDD"))
```
_________________________________________________________________________________________________________________________________________________________________

### MSDD Volume Graphs
```{r MSDD Metrics Output4, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
print(vol_trend_site_graph(site = "MSDD"))
print(vol_breakdown_site_graph(site = "MSDD"))
```

:::: {.blackbox data-latex=""}
::: {.center data-latex=""}
<font size="4">**For `r vol_takeaways_function(site = "MSDD")[1]` in `r full_month` `r full_year`:**</font>
:::

<font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "MSDD")[2]`**, reaching **`r vol_takeaways_function(site = "MSDD")[3]`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "MSDD")[4]`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "MSDD")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "MSDD")[6]`** of 2019 monthly average.</font><br/>
::::
_________________________________________________________________________________________________________________________________________________________________

### MSDD Volume by Dept
<!-- <span style='font-size: 16px; color: black'>**MSDD `r todays_month`-`r todays_year` MTD Volume by Department**</span><br/> -->
<!-- <span style='font-size: 14px; color: black'>*Includes visits up to `r date_end`</span><br/> -->
<span style='font-size: 16px; color: black'>**MSDD `r full_month`-`r full_year` Volume by Department**</span><br/>
```{r MSDD Metrics Output5, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
monthly_vol_dept_function(site = "MSDD")
```
_________________________________________________________________________________________________________________________________________________________________
...
<font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "MSH- AMBULATORY CARE")[2]`**, reaching **`r vol_takeaways_function(site = "MSH- AMBULATORY CARE")[3]`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "MSH- AMBULATORY CARE")[4]`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "MSH- AMBULATORY CARE")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "MSH- AMBULATORY CARE")[6]`** of 2019 monthly average.</font><br/>
::::
_________________________________________________________________________________________________________________________________________________________________

### MSH- AMBULATORY CARE Volume by Dept
...
<font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "MSH-MSDFP")[2]`**, reaching **`r vol_takeaways_function(site = "MSH-MSDFP")[3]`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "MSH-MSDFP")[4]`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "MSH-MSDFP")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "MSH-MSDFP")[6]`** of 2019 monthly average.</font><br/>
::::
_________________________________________________________________________________________________________________________________________________________________
...
<!-- ## MSHP {.tabset .tabset-fade .tabset-pills} -->
<!-- ### MSHP Daily Volume -->
<!-- ```{r MSHP Metrics Output1, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'} -->
<!-- print(daily_vol_site_function(site = "MSHP")) -->
<!-- print(daily_vol_last_year_site_function(site = "MSHP")) -->

<!-- ``` -->
<!-- _________________________________________________________________________________________________________________________________________________________________ -->

<!-- ### MSHP Weekly Volume -->
<!-- ```{r MSHP Metrics Output2, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'} -->
<!-- print(weekly_vol_site_function(site = "MSHP")) -->
<!-- print(weekly_vol_last_year_site_function(site = "MSHP")) -->
<!-- ``` -->
<!-- _________________________________________________________________________________________________________________________________________________________________ -->

<!-- ### MSHP Monthly Volume -->
<!-- ```{r MSHP Metrics Output3, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'} -->
<!-- print(monthly_vol_site_function(site = "MSHP")) -->
<!-- ``` -->
<!-- _________________________________________________________________________________________________________________________________________________________________ -->

<!-- ### MSHP Volume Graphs -->
<!-- ```{r MSHP Metrics Output4, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'} -->
<!-- print(vol_trend_site_graph(site = "MSHP")) -->
<!-- print(vol_breakdown_site_graph(site = "MSHP")) -->
<!-- ``` -->

<!-- :::: {.blackbox data-latex=""} -->
<!-- ::: {.center data-latex=""} -->
<!-- <font size="4">**For `r vol_takeaways_function(site = "MSHP")[1]` in `r full_month` `r full_year`:**</font> -->
<!-- ::: -->

<!-- <font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "MSHP")[2]`**, reaching **`r vol_takeaways_function(site = "MSHP")[3]`** of `r full_month` `r last_year`.</font><br/> -->
<!-- <font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "MSHP")[4]`** of `r full_month` `r last_year` volume.</font><br/> -->
<!-- <font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "MSHP")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "MSHP")[6]`** of 2019 monthly average.</font><br/> -->
<!-- :::: -->
<!-- _________________________________________________________________________________________________________________________________________________________________ -->

<!-- ### MSHP Volume by Dept -->
<!-- <!-- <span style='font-size: 16px; color: black'>**MSHP `r todays_month`-`r todays_year` MTD Volume by Department**</span><br/> --> -->
<!-- <!-- <span style='font-size: 14px; color: black'>*Includes visits up to `r date_end`</span><br/> --> -->
<!-- <span style='font-size: 16px; color: black'>**MSHP `r full_month`-`r full_year` Volume by Department**</span><br/> -->
<!-- ```{r MSHP Metrics Output5, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'} -->
<!-- monthly_vol_dept_function(site = "MSHP") -->
<!-- ``` -->
_________________________________________________________________________________________________________________________________________________________________
...
<font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "MSM")[2]`**, reaching **`r vol_takeaways_function(site = "MSM")[3]`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "MSM")[4]`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "MSM")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "MSM")[6]`** of 2019 monthly average.</font><br/>
::::
_________________________________________________________________________________________________________________________________________________________________
...
<font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "MSUS")[2]`**, reaching **`r vol_takeaways_function(site = "MSUS")[3]`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "MSUS")[4]`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "MSUS")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "MSUS")[6]`** of 2019 monthly average.</font><br/>
::::
_________________________________________________________________________________________________________________________________________________________________
...
<font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "MSW")[2]`**, reaching **`r vol_takeaways_function(site = "MSW")[3]`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "MSW")[4]`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "MSW")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "MSW")[6]`** of 2019 monthly average.</font><br/>
::::
_________________________________________________________________________________________________________________________________________________________________
...
<font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "NETWORK")[2]`**, reaching **`r vol_takeaways_function(site = "NETWORK")[3]`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "NETWORK")[4]`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "NETWORK")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "NETWORK")[6]`** of 2019 monthly average.</font><br/>
::::
_________________________________________________________________________________________________________________________________________________________________

### NETWORK Volume by Dept
...
### ONCOLOGY Daily Volume
```{r Oncology Metrics Output1, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
print(daily_vol_site_function(site = "ONCOLOGY"))
print(daily_vol_last_year_site_function(site = "ONCOLOGY"))
...
### ONCOLOGY Weekly Volume
```{r Oncology Metrics Output2, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
print(weekly_vol_site_function(site = "ONCOLOGY"))
print(weekly_vol_last_year_site_function(site = "ONCOLOGY"))
```

_________________________________________________________________________________________________________________________________________________________________
*End of Report.*

### ONCOLOGY Monthly Volume
```{r Oncology Metrics Output3, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
print(monthly_vol_site_function(site = "ONCOLOGY"))
```

_________________________________________________________________________________________________________________________________________________________________
*End of Report.*

### ONCOLOGY Volume Graphs
```{r Oncology Metrics Output4, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
print(vol_trend_site_graph(site = "ONCOLOGY"))
print(vol_breakdown_site_graph(site = "ONCOLOGY"))
```

:::: {.blackbox data-latex=""}
::: {.center data-latex=""}
<font size="4">**For `r vol_takeaways_function(site = "ONCOLOGY")[1]` in `r full_month` `r full_year`:**</font>
:::

<font size="4">--  Total ambulatory encounters were **`r vol_takeaways_function(site = "ONCOLOGY")[2]`**, reaching **`r vol_takeaways_function(site = "ONCOLOGY")[3]`** of `r full_month` `r last_year`.</font><br/>
<font size="4">--  In-person encounters were at **`r vol_takeaways_function(site = "ONCOLOGY")[4]`** of `r full_month` `r last_year` volume.</font><br/>
<font size="4">--  Total ambulatory encounters were at **`r vol_takeaways_function(site = "ONCOLOGY")[5]`** of `r last_year` monthly average and **`r vol_takeaways_function(site = "ONCOLOGY")[6]`** of 2019 monthly average.</font><br/>
::::
_______________________________________________________________________________________________________________________________________________________________

### ONCOLOGY Volume by Dept
<!-- <span style='font-size: 16px; color: black'>**ONCOLOGY `r todays_month`-`r todays_year` MTD Volume by Department**</span><br/> -->
<!-- <span style='font-size: 14px; color: black'>*Includes visits up to `r date_end`</span><br/> -->
<span style='font-size: 16px; color: black'>**ONCOLOGY `r full_month`-`r full_year` Volume by Department**</span><br/>
```{r ONCOLOGY Metrics Output5, fig.height = 7, fig.width = 13, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
monthly_vol_dept_function(site = "ONCOLOGY")
```
_________________________________________________________________________________________________________________________________________________________________
*End of Report.*
