...
scheduling_data_raw <- readRDS("/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/historical_data_12.20.2022.rds") %>%
  # filter(!is.na(Campus)) %>%
  # filter(Campus %in% c("NETWORK","MSM","MSH-MSDFP","ONCOLOGY","MSW","MSBI","MSUS","MSH- AMBULATORY CARE","MSDD")) %>%
  filter(Appt.Status == "Arrived")%>%
  filter(Appt.DateYear >= as.Date("2022-12-01", format="%Y-%m-%d") & Appt.DateYear <= as.Date(max(Appt.Made.DTTM), format="%Y-%m-%d"))
...
# }
  ...
  daily_vol <- scheduling_data %>%
  filter(Appt.Status == "Arrived") %>%
  filter(Appt.DateYear >= todays_date-7 & Appt.DateYear < todays_date+8) %>%
  group_by(Campus, Appt.Year, Appt.WeekNum, Appt.DateYear, Appt.Day, Visit.Method) %>%
  summarise(daily_vol = n()) %>%
  pivot_wider(
    names_from = Visit.Method,
...
# Volume Breakdown by Month 
   ...
   ggplot(vol_breakdown_mshs, aes(x= factor(Appt.Month, levels = monthOptions), 
      ...
      labs(title = paste0("MSHS Ambulatory Arrived Volume Breakdown by Month"),
           # subtitle = paste0("Based on data from 2021-01-01 to 2022-10-31","\n"),
           subtitle = paste0("Based on data from 2021-01-01 to ",max(scheduling_data$Appt.DateYear),"\n"),
     ...
     # labs(caption = paste0("*2019 Monthly Average: ", prettyNum(avg_2019, big.mark = ','),"\n**Includes Rad Onc data up to October, 2022", " and Radiology data up to October, 2022"))+
        labs(caption = paste0("*2019 Monthly Average: ", prettyNum(avg_2019, big.mark = ','),"\n**Includes Rad Onc data up to ", rad_onc_date, " and Radiology data up to ",radiology_date))+
