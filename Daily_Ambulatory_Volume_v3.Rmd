...
# scheduling_data_raw <- readRDS("/SharedDrive/deans/Presidents/SixSigma/Individual Folders/Current Employees/Engineers/So Youn Kweon/server-upload/historical_data_11.2.2022.rds") %>%
#   filter(!is.na(Campus)) %>%
...
scheduling_data_raw <- readRDS("/nfs/data/Applications/Ambulatory/Data_Updated/historical_data.rds") %>%
  ...
  filter(Campus %in% c("NETWORK","MSM","MSH-MSDFP","ONCOLOGY","MSW","MSBI","MSUS","MSH- AMBULATORY CARE","MSDD")) %>%
  filter(Appt.Status == "Arrived")%>%
  filter(Appt.DateYear >= as.Date("2021-01-01", format="%Y-%m-%d") & Appt.DateYear <= as.Date("2021-11-05", format="%Y-%m-%d"))
...
# rad_onc_data_raw <- read.csv("~/server-upload/Rad_Onc_Data.csv")
# radiology_data_raw <- read.csv("~/server-upload/Radiology_Data.csv")
# radiology_ref <- read.csv("~/server-upload/Radiology_Site_Mapping.csv")
```
...
# }
  ...
  daily_vol <- scheduling_data %>%
  filter(Appt.Status == "Arrived") %>%
  filter(Appt.DateYear >= todays_date-14 & Appt.DateYear < todays_date) %>%
  group_by(Campus, Appt.Year, Appt.WeekNum, Appt.DateYear, Appt.Day, Visit.Method) %>%
  summarise(daily_vol = n()) %>%
  pivot_wider(
    names_from = Visit.Method,
...
# Volume Breakdown by Month 
   ...
   ggplot(vol_breakdown_mshs, aes(x= factor(Appt.Month, levels = monthOptions), 
      ...
      labs(title = paste0("MSHS Ambulatory Arrived Volume Breakdown by Month"),
           subtitle = paste0("Based on data from 2021-01-01 to 2022-09-30","\n"),
           # subtitle = paste0("Based on data from 2021-01-01 to ",max(scheduling_data$Appt.DateYear),"\n"),
     ...
     # geom_text(data = ann_text_2019, aes(label = lab), vjust=-0.5, size=3.5, fontface="bold")+
          geom_hline(yintercept=avg_2021, linetype = "dotted", color="red", size=0.8) +
          geom_text(data = ann_text_2021, aes(label = lab), size=3.5, fontface="bold", vjust = -1, nudge_x = 1, color = "red")+
     labs(caption = paste0("*2019 Monthly Average: ", prettyNum(avg_2019, big.mark = ','),"\n**Includes Rad Onc data up to September, 2022", " and Radiology data up to September, 2022"))+
...
total_amb_prior_year <- percent((current_encounters-avg_2021)/avg_2021,0)
...
total_amb_prev_year <- percent((current_encounters-avg_2019)/avg_2019,0)
...
vol_takeaways_function <- function(site){
  # site <- "MSH-MSDFP"
    ...
    total_in_person_site <- percent((monthly_vol_site %>% filter(Visit.Method == "IN PERSON") %>% filter(Appt.Month == full_month) %>%
                                       mutate(perc_diff =  (`2022`-`2021`)/`2021`))$perc_diff,0)
    ...
    total_amb_prior_year_site <- percent((current_encounters_site-avg_2021_site)/avg_2021_site,0)
...
:::
<font size="4">--  Total ambulatory encounters were `r vol_takeaways_function(site = "MSDD")[2]`. This is `r vol_takeaways_function(site = "MSDD")[3]`.</font><br/>
<font size="4">--  In-person encounters `r vol_takeaways_function(site = "MSDD")[4]`.</font><br/>
<font size="4">--  Total ambulatory encounters `r vol_takeaways_function(site = "MSDD")[5]` from `r last_year` monthly average and `r vol_takeaways_function(site = "MSDD")[6]` from 2019 monthly average.
::::
_________________________________________________________________________________________________________________________________________________________________
### MSDD Volume by Dept
...
:::
<font size="4">--  Total ambulatory encounters were `r vol_takeaways_function(site = "ONCOLOGY")[2]`. This is `r vol_takeaways_function(site = "ONCOLOGY")[3]`.</font><br/>
<font size="4">--  In-person encounters `r vol_takeaways_function(site = "ONCOLOGY")[4]`.</font><br/>
<font size="4">--  Total ambulatory encounters `r vol_takeaways_function(site = "ONCOLOGY")[5]` from `r last_year` monthly average and `r vol_takeaways_function(site = "ONCOLOGY")[6]` from 2019 monthly average.
::::
